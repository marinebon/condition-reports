...
if (!file.exists("functional_diversity/func_dist_rds")){
  ...
  trait_matrix = read_csv('functional_diversity/species_trait_matrix_316_spp_10_25_17.csv') #316 species
...
trait_matrix = read_csv('functional_diversity/species_trait_matrix_316_spp.csv') #316 species
...
if(!file.exists("psu_abun_sum_rds")){
  ...
  abun_spp = psu_fk_abun_no_spp %>% filter(!abundance == "0")
  spp_316 = unique(abun_spp$SPECIES_CD) %>% #316 species
    as.data.frame()
  colnames(spp_316)[1] = "SPECIES_CD"
...
#INPR 2002 PSU 472U detected abundance sum 10,414 species (abnormally large)
INPR_2002 = psu_fk_abun_no_spp %>%
  filter(YEAR == "2002") %>%
  filter(STRAT == "INPR") %>%
  filter(PRIMARY_SAMPLE_UNIT == "472U") %>%
  arrange(desc(abundance))
#HYP HARR abundance 9999.5

#OFPR 2003 PSU 119U detected abundance sum 5,698 species (abnormally large)
OFPR_2003 = psu_fk_abun_no_spp %>%
  filter(YEAR == "2003") %>%
  filter(STRAT == "OFPR") %>%
  filter(PRIMARY_SAMPLE_UNIT == "119U") %>%
  arrange(desc(abundance))
#DEC PUNC abundance 5000.0

#MCPR 2004 PSU 403U detected abundance sum 5,698 species (abnormally large)
MCPR_2004 = psu_fk_abun_no_spp %>%
  filter(YEAR == "2004") %>%
  filter(STRAT == "MCPR") %>%
  filter(PRIMARY_SAMPLE_UNIT == "403U") %>%
  arrange(desc(abundance))
#HYP HARR abundance 5000.0

FMLR_2001 = psu_fk_abun_no_spp %>%
  filter(YEAR == "2001") %>%
  filter(STRAT == "FMLR") %>%
  filter(PRIMARY_SAMPLE_UNIT == "220U") %>%
  arrange(desc(abundance))
#ATH STIP abundance 10,000.0 Hardhead Silverside, Atherinidae Atherinomorus

FDLR_2002 = psu_fk_abun_no_spp %>%
  filter(YEAR == "2001") %>%
  filter(STRAT == "FDLR") %>%
  filter(PRIMARY_SAMPLE_UNIT == "120U") %>%
  arrange(desc(abundance))
#COR PERS 1,690 #Masked Goby, Coryphopterus personatus
...
if(!file.exists("psu_bio_sum_rds")){
  ...
  psu_fk_bio_no_spp = psu_fk_bio %>%
    filter(!stringr::str_detect(SPECIES_CD, 'SPE\\.$')) %>% #328 species  
    filter(stringr::str_detect(protected_status, 'all'))  
  ...
  bio_spp = psu_fk_bio_no_spp %>% filter(!biomass == "0")
  length(unique((bio_spp$SPECIES_CD))) #308 species
...
if(!file.exists("fd_eveness_rds")){
  ...
  #read in trait matrix
  traits = read.csv('functional_diversity/species_trait_matrix_316_spp_10_25_17.csv')
...
if(!file.exists("psu_matrices_data_rds")){
  ...
  saveRDS(all_matrices_data, "psu_matrices_data_rds")
    ...
    write_csv(all_matrices_data, "big_csv/all_matrices_data.csv")
  }
...
write_csv(mean_indices, "mean_indices.csv")
